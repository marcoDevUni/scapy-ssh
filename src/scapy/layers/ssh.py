...
SSH_TYPE_BOOL = {0x00: True, 0xFF: False}
...
def ssh_name_list(name, fmt="!I", numbytes=None, default=""):
    return [
        XBLenField(
            "%s_length" % name, None, length_of="%s" % name, fmt=fmt, numbytes=numbytes
        ),
        StrLenField(
            "%s" % name, default, length_from=lambda x: getattr(x, "%s_length" % name)
        ),
    ]
...
class SSHMessage(Packet):
    ...
    fields_desc = [
        XBLenField(
            ...
            None,
            fmt="!I",
            adjust=lambda pkt, x: x + 2 if pkt.lastlayer().haslayer(Raw) else x + 2,
        ),
        XBLenField(
            "padding_length",
            None,
            fmt="!I",
            adjust=lambda pkt, x: len(pkt.lastlayer())
...
class SSHKexInit(Packet):
    ...
    fields_desc = (
        ...
        + ssh_name_list(
            "server_host_key_algorithms", default=",".join(SSH_ALGO_HOSTKEY)
        )
        + ssh_name_list(
            "encryption_algorithms_client_to_server", default=",".join(SSH_ALGO_CIPHERS)
        )
        + ssh_name_list(
            "encryption_algorithms_server_to_client", default=",".join(SSH_ALGO_CIPHERS)
        )
        + ssh_name_list(
            "mac_algorithms_client_to_server", default=",".join(SSH_ALGO_HMACS)
        )
        + ssh_name_list(
            "mac_algorithms_server_to_client", default=",".join(SSH_ALGO_HMACS)
        )
        + ssh_name_list(
            "compression_algorithms_client_to_server",
            default=",".join(SSH_ALGO_COMPRESSION),
        )
        + ssh_name_list(
